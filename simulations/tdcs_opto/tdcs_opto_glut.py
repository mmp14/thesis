...
def f(t, y, opto_amplitude, tdcs_amp):
    ...
    #P1
    f1 = y6
    f2 = a_a * A_a * utils.sigma(C1 * y2 + C2 * y3 + C3 * (A_a / a_a) * phi_e1 + C11 * y4 + y11 + utils.tdcs(t, tdcs_amp)) - 2 * a_a * y6 - pow(a_a, 2) * y1
    #SS 
    f3 = y7
    ...
    #P2
    f7 = y9
    f8 = a_a * A_a * utils.sigma(C6 * y4 + C7 * y5 + C8 * (A_a / a_a) * (phi_e2) + C12 * y1 + y11 + utils.tdcs(t, tdcs_amp),
...
def solve_stepwise(f, Y_init, opto_amp, tdcs_amp):
    args = (opto_amp, tdcs_amp,)
