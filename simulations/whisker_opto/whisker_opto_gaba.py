...
def f(t, y, opto_amplitude):
    ...
    # SST (slow inhibitory)
    f5 = y8
    f6 = a_gs * A_gs * utils.sigma(C5 * y1 + y13) - 2 * a_gs * y8 - pow(a_gs, 2) * y3
    # P2
    f7 = y9
    ...
    # PV (fast inhibitory)
    f9 = y10
    f10 = a_gf * A_gf * utils.sigma(C9 * y4 + C10 * y5 + C13 * y1 + y13) - 2 * a_gf * y10 - pow(a_gf, 2) * y5
    # whisker stimulation
    f11 = y12
...
for a in opto_amplitudes:
    ...
    P1, P2, SS, SST, PV = utils.get_population_activities_with_whisker_opto_gaba(sol_combined, C1, C2, C4, C5, C6, C7, C9, C10, C11, C12, C13)
...
# Plot #
#########
colour = ["black", "lightgreen", "seagreen"]
