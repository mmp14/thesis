...
def f(t, y, opto_amplitude):
    ...
    #P1
    f1 = y6
    f2 = a_a * A_a * utils.sigma(C1 * y2 + C2 * y3 + C3 * (A_a / a_a) * phi_e1 + C11 * y4 + y11) - 2 * a_a * y6 - pow(a_a, 2) * y1
    #SS 
    f3 = y7
    f4 = a_a * A_a * utils.sigma(C4 * y1 + y11) - 2 * a_a * y7 - pow(a_a, 2) * y2
    #SST (slow inhibitory)
    f5 = y8
...
for a in opto_amplitudes:
    ...
    P1, P2, SS, SST, PV = utils.get_population_activities_with_opto_glut(sol_combined, C1, C2, C4, C5, C6, C7, C9, C10, C11, C12, C13)
...
# Plot #
#########
colour = ["xkcd:soft pink", "deeppink"]
