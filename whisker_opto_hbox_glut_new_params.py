...
from param.params import (
    A_a,a_a,A_gs,a_gs,A_gf,a_gf,A_o,a_o,A_w,a_w,
    C1,C2,C3,C4,C5,C6,C7,C8,C9,C10,C11,C12,C13,
    phi_e1,phi_e2,v0_p2
)

opto_amplitudes = [0, .25, .5]

def f(t, y, opto_amplitude):
    (y1,y6,y2,y7,y3,y8,y4,y9,y5,y10,y11,y12,y13,y14) = y

    # the model equations
    # P1
    f1 = y6
    f2 = a_a * A_a * utils.sigma(C1 * y2 + C2 * y3 + C3 * (A_a / a_a) * phi_e1 + C11 * y4  + y13,
                           ) - 2 * a_a * y6 - pow(a_a, 2) * y1
    # SS
    f3 = y7
    f4 = a_a * A_a * utils.sigma(C4 * y1 + y13 ) - 2 * a_a * y7 - pow(a_a, 2) * y2
    # SST (slow inhibitory)
    f5 = y8
    f6 = a_gs * A_gs * utils.sigma(C5 * y1) - 2 * a_gs * y8 - pow(a_gs, 2) * y3
    # P2
    f7 = y9
    f8 = a_a * A_a * utils.sigma(C6 * y4 + C7 * y5 + C8 * (A_a / a_a) * phi_e2 + C12 * y1 + y11 + y13,
    ...
    # opto stimulation
    f13 = y14
    f14 = a_o * A_o * utils.sigma(utils.opto(t, opto_amplitude)) - 2 * a_o * y14 - pow(a_o, 2) * y13

    return [f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14]

def solve_stepwise(f, Y_init, opto_amplitude):
    args = (opto_amplitude,)
...
# Run simulation
# -----------------
Y_init = np.zeros(14)  # initial condition vector

LFP_array= []
pop_potentials = [[] for i in range(5)] # one list per pop
pop_rates = [[] for i in range(5)] # one list per pop
y_arrays = [[] for _ in range(14)]  # one list per state variab
t_arrays = [[] for _ in range(14)]  # one list per state variab


for a in opto_amplitudes:

    t_combined, sol_combined = solve_stepwise(f, Y_init, a)
    ...
    P1, P2, SS, SST, PV = utils.get_population_activities_with_whisker_opto_glut(sol_combined, C1, C2, C4, C5, C6, C7, C9, C10, C11, C12, C13)
...
# Plot #
#########
colour = ["black", "xkcd:soft pink", "deeppink"]

handles, labels = utils.plot_LFPs(opto_amplitudes, LFP_array, times, colour)

utils.plot_potentials(opto_amplitudes, pop_potentials, times, colour, handles, labels)

utils.plot_firing_rates(opto_amplitudes, pop_rates, times, colour, handles, labels)

utils.plot_y(opto_amplitudes, y_arrays, t_arrays, colour)