...
def f(t, y, tdcs_amp):
    (y1,y6,y2,y7,y3,y8,y4,y9,y5,y10,yw,yww) = y

    # the model equations
    # P1
    f1 = y6
    f2 = a_a * A_a * utils.sigma(C1 * y2 + C2 * y3 + C3 * (A_a / a_a) * phi_e1 + C11 * y4 + utils.tdcs(t, tdcs_amp)) - 2 * a_a * y6 - pow(a_a, 2) * y1
    # SS
    f3 = y7
    ...
    # P2
    f7 = y9
    f8 = a_a * A_a * utils.sigma(C6 * y4 + C7 * y5 + C8 * (A_a / a_a) * phi_e2 + C12 * y1 + yw + utils.tdcs(t, tdcs_amp),
    ...
    # whisker stimulation
    f11 = yww
    f12 = a_w * A_w * utils.sigma(utils.whisker(t)) - 2 * a_w * yww - pow(a_w, 2) * yw

    return [f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12]

def solve_stepwise(f, Y_init, tdcs_amp):
    args = (tdcs_amp,)
